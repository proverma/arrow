...
TestExecutor.prototype.startProxy = function(proxyConfigPath, callback) {
    ...
    var proxyManager = new ProxyManager(path.resolve(global.workingDirectory, proxyConfigPath),sf.config, testExecutor.getProxyLogFileName());
 ...
 * @returns {string}
 */
TestExecutor.prototype.getProxyLogFileName = function() {
    ...
    function incrementTestSessionCount(test) {
        if (test.descriptorPath) {
            if (sf.descriptorObj[test.descriptorPath]) {
                sf.descriptorObj[test.descriptorPath].testSessionCount += 1;
            } else {
                sf.descriptorObj[test.descriptorPath].testSessionCount = 1;
            }
            ...
            if (sf.proxyConfigList && sf.proxyConfigList.length > 0) {
                ...
                async.eachSeries(sf.proxyConfigList, testExecutor.startProxy, function(err) {
                    ...
                    if (err) {
                        ...
                        // Create log file for proxy
                        try {
                            var proxyLogFile = self.getProxyLogFileName();
                            self.logger.debug('Creating proxy log file ' + proxyLogFile);
                            sf.proxyLogHandler = fs.openSync(proxyLogFile,"w");
                        }
                        catch(e) {
                            self.logger.error('Error in creating stream for proxy log, error::' + e);
                        }
