...
TestExecutor.prototype.writeReportForDescriptor = function(testSession, callback) {
    ...
    for (descriptor in sf.descriptorObj) {
        ...
        if (descriptor === testSession.qualifiedDescriptorPath) {
            ...
            if (sf.descriptorObj[descriptor].testSessionCount === 0) {
                ...
                sf.timeReportObj[testSession.qualifiedDescriptorPath].endTime = Date.now(); // TODO Is endTime required ???
                sf.timeReportObj[testSession.qualifiedDescriptorPath].timeTaken =
                    ((sf.timeReportObj[testSession.qualifiedDescriptorPath].endTime - sf.timeReportObj[testSession.qualifiedDescriptorPath].startTime) / 1000).toFixed(2);

                testExecutor.logger.info('Time taken to run descriptor-' + testSession.qualifiedDescriptorPath + ' = ' + sf.timeReportObj[testSession.qualifiedDescriptorPath].timeTaken);

                testExecutor.logger.info('All tests over for descriptor:' + testSession.qualifiedDescriptorPath);
                ...
                reportObj = {
                    ...
                    "dataDriverKey":sf.descriptorObj[descriptor].dataDriverKey
                };
                reportManager = new ReportManager(reportObj);
                reportManager.totalTimeTaken = sf.timeReportObj[testSession.qualifiedDescriptorPath].timeTaken;
...
TestExecutor.prototype.runNextTest = function(testQueue) {
    ...
    if (!sf.timeReportObj[testSession.qualifiedDescriptorPath]) {
        sf.timeReportObj[testSession.qualifiedDescriptorPath] = [];
        sf.timeReportObj[testSession.qualifiedDescriptorPath].startTime = Date.now();
    }
    ...
    testSession.runTest(function (error) {
        ...
        if (error) {
            ...
            testExecutor.logger.error("Error : " + error);
        }
        ...
        if (sf.timeReportObj[thisSession.qualifiedDescriptorPath].testSessionTimeArr) {
            testSessionTimeArr = sf.timeReportObj[thisSession.qualifiedDescriptorPath].testSessionTimeArr;
        } else {
            testSessionTimeArr = {};
        }
        ...
        sf.timeReportObj[thisSession.qualifiedDescriptorPath].testSessionTimeArr = testSessionTimeArr;

        testExecutor.logger.info('Time taken to run - ' + thisSession.qualifiedDescriptorPath + '-' + thisSession.args.testName + ' is::' +
...
TestExecutor.prototype.setupTestQueues = function(callback) {
    ...
    var test,
        ...
        i,
        j;
    function incrementTestSessionCount(test) {
        if (test.qualifiedDescriptorPath) {
            if (sf.descriptorObj[test.qualifiedDescriptorPath]) {
                sf.descriptorObj[test.qualifiedDescriptorPath].testSessionCount += 1;
            } else {
                sf.descriptorObj[test.qualifiedDescriptorPath].testSessionCount = 1;
            }
        ...
        testExecutor.populateProxyConfigList(function () {
            ...
            if (sf.proxyConfigList && sf.proxyConfigList.length > 0) {
                ...
                async.eachSeries(sf.proxyConfigList, testExecutor.startProxy, function(err) {
                    ...
                    if (err) {
                        testExecutor.logger.error('ERROR !! Failed to start proxy servers..');
                    } else {
...
TestExecutor.prototype.executeTests = function () {
    ...
    if (sf.reuseSession) {
        ...
        hub.getSessions(function (error, arrSessions) {
            if (error) {
                self.logger.fatal('No Reusable session found!');
                process.exit(1);
            }
