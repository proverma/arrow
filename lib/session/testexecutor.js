...
TestExecutor.prototype.startProxy = function(proxyConfigPath, callback) {
    ...
    if (global.workingDirectory) {
        proxyFile = path.resolve(global.workingDirectory, proxyFile);
    }

    var proxyManager = new ProxyManager(path.resolve(global.workingDirectory, proxyConfigPath),sf.config, proxyFile);
    ...
    if (sf.args.startProxyServer === true || sf.args.startProxyServer === undefined) {
        ...
        testExecutor.populateProxyConfigList(function () {
            ...
            if (sf.proxyConfigList && sf.proxyConfigList.length > 0) {
                ...
                sf.startProxy = true;
                // Create write stream for proxy log
                var proxyFile;
                if (global.workingDirectory) {
                    proxyFile = path.resolve(global.workingDirectory, "proxy.log");
                } else {
                    proxyFile = "proxy.log";
                }

                try {
                    self.logger.debug('Creating write stream for ' + proxyFile);
                    sf.proxyLogHandler = fs.openSync(proxyFile,"w");
//                    sf.proxyStream = fs.createWriteStream(proxyFile, { flags : 'a' });
                }
                catch(e) {
