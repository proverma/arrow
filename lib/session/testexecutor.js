...
TestExecutor.prototype.runNextTest = function(testQueue) {
    ...
    if ((testQueue.curIndex + 1) === testQueue.sessions.length) {
    ...
    testSession.runTest(function (error) {
        ...
        testExecutor.logger.info('Time taken to run - ' + thisSession.args.testName + ' is::' +
...
TestExecutor.prototype.setupTestQueues = function(callback) {
    ...
    if (wdSessions.length > 0) {
        ...
        for (i = 0; i < wdSessions.length; i += 1) {
            testQueues[i] = {curIndex: -1, sessions: []};
        ...
        testQueues[0] = {curIndex: -1, sessions: []};
        for (i = 0; i < tests.length; i += 1) {
            ...
            if (browsers.length > 1) {
                ...
                if (sf.descriptorMap[test.descriptorPath]) {
                    sf.descriptorMap[test.descriptorPath] = sf.descriptorMap[test.descriptorPath] * browsers.length;
                }
            }
