...
//    var thisSession = testQueue.sessions[testQueue.curIndex];
    ...
    testSession.runTest(function (error) {
        ...
        if (error) {
            var obj = {};
            obj.error = error;
            console.log('\n*****Adding error for test :' + testSession.descriptorPath + ' - ' + testSession.args.testName);
...
//            testExecutor.logger.debug("Error - Callback of runTest : " + error);
        ...
        if (sf.timeReportObj[this.descriptorPath].testSessionTimeArr) {
            testSessionTimeArr = sf.timeReportObj[this.descriptorPath].testSessionTimeArr;
        } else {
            testSessionTimeArr = {};
        }
        ...
        testSessionTimeArr[testSession.args.testName].timeTaken = ((testSession.endTime - this.startTime) / 1000).toFixed(2);
        sf.timeReportObj[testSession.descriptorPath].testSessionTimeArr = testSessionTimeArr;

        testExecutor.logger.info('Time taken to run - ' + testSession.descriptorPath + '-' + testSession.args.testName + ' is::' +
