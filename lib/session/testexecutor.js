...
//                console.log('\n\n\n***Before writing report for ' + testSession.descriptorPath + ' , test Session is :' + util.inspect(testSession));
                ...
                testExecutor.logger.info('\n\nTime taken to run descriptor-' + testSession.descriptorPath + ' = ' + sf.timeReportObj[testSession.descriptorPath].timeTaken);
                ...
                console.log('***Calling writeReports for descriptor ' + testSession.descriptorPath + ' from testExecutor..');
...
//    var thisSession = testQueue.sessions[testQueue.curIndex];
    ...
    console.log('\n*****Running test :' + testSession.descriptorPath + ' - ' + testSession.args.testName);
    ...
    testSession.runTest(function (error) {
        ...
        if (error) {
            var obj = {};
            obj.error = error;
            console.log('\n*****Adding error for test :' + self.descriptorPath + ' - ' + self.args.testName);
            self.driver.reports.addReport(obj);
//            testExecutor.logger.debug("Error - Callback of runTest : " + error);
            testExecutor.logger.debug("Obj - Callback of runTest : " + JSON.stringify(obj));
        }
        ...
        if (sf.timeReportObj[self.descriptorPath].testSessionTimeArr) {
            testSessionTimeArr = sf.timeReportObj[self.descriptorPath].testSessionTimeArr;
        } else {
            testSessionTimeArr = {};
        }

        testSessionTimeArr[self.args.testName] = {};

        testSessionTimeArr[self.args.testName].timeTaken = ((self.endTime - self.startTime) / 1000).toFixed(2);
        sf.timeReportObj[self.descriptorPath].testSessionTimeArr = testSessionTimeArr;

        testExecutor.logger.info('Time taken to run - ' + self.descriptorPath + '-' + self.args.testName + ' is::' +
            testSessionTimeArr[self.args.testName].timeTaken);

        testExecutor.writeReportForDescriptor(self, function() {
            curSessionCount += 1;
