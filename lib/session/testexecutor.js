...
TestExecutor.prototype.writeReportForDescriptor = function(testSession, callback) {
    ...
    for (descriptor in sf.descriptorObj) {
        ...
        if (descriptor === testSession.descriptorName) {
            ...
            if (sf.descriptorObj[descriptor].testSessionCount === 0) {
                ...
                sf.timeReportObj[testSession.descriptorName].endTime = Date.now(); // TODO Is endTime required ???
                sf.timeReportObj[testSession.descriptorName].timeTaken =
                    ((sf.timeReportObj[testSession.descriptorName].endTime - sf.timeReportObj[testSession.descriptorName].startTime) / 1000).toFixed(2);

                testExecutor.logger.info('Time taken to run descriptor-' + testSession.descriptorName + ' = ' + sf.timeReportObj[testSession.descriptorName].timeTaken);

                testExecutor.logger.info('All tests over for descriptor:' + testSession.descriptorName);
                reportObj = {
                    "reportFolder" : global.reportFolder,
                    "arrTestSessions" : sf.testSessionMap[descriptor],
                    "descriptor" : testSession.descriptorName,
                ...
                reportManager.totalTimeTaken = sf.timeReportObj[testSession.descriptorName].timeTaken;
...
TestExecutor.prototype.runNextTest = function(testQueue) {
    ...
    if (!sf.timeReportObj[testSession.descriptorName]) {
        sf.timeReportObj[testSession.descriptorName] = [];
        sf.timeReportObj[testSession.descriptorName].startTime = Date.now();
    }
    ...
    testSession.runTest(function (error) {
        ...
        if (sf.timeReportObj[thisSession.descriptorName].testSessionTimeArr) {
            testSessionTimeArr = sf.timeReportObj[thisSession.descriptorName].testSessionTimeArr;
        } else {
            testSessionTimeArr = {};
        }
        ...
        sf.timeReportObj[thisSession.descriptorName].testSessionTimeArr = testSessionTimeArr;

        testExecutor.logger.info('Time taken to run - ' + thisSession.descriptorName + '-' + thisSession.args.testName + ' is::' +
...
TestExecutor.prototype.setupTestQueues = function(callback) {
    ...
    function incrementTestSessionCount(test) {
        if (test.descriptorName) {
            if (sf.descriptorObj[test.descriptorName]) {
                sf.descriptorObj[test.descriptorName].testSessionCount += 1;
            } else {
                sf.descriptorObj[test.descriptorName].testSessionCount = 1;
            }
