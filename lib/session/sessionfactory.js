...
SessionFactory.prototype.getFactoryTests = function () {
    ...
    for (i = 0; i < this.arrDescriptor.length; i += 1) {
        ...
        for (j = 0; j < testJsonArr.length; j+=1) {
            ...
            descriptorName = descFile;
            if (isDataDriven){
                descriptorName += ( ' - ' + testJson.name);
            }

            if (!this.descriptorObj[descriptorName]) {
                this.descriptorObj[descriptorName] = {};
                this.descriptorObj[descriptorName].testSessionCount = 0;
            }

            this.descriptorObj[descriptorName].testSuiteName = testJson.name;
...
//        this.descLib = "";
            ...
            for (testName in dp) {
                ...
                testData.descriptorName = descriptorName; // Needed for data driven descriptor e.g contains descriptor name appended with some key e.g descriptor-0 TODO
...
//            }
                ...
                if (testData.config.descriptorSharedParams) {
...
SessionFactory.prototype.tearDown = function (testQueue, wdSessions) {
    ...
    if (this.args.report && testQueue.sessions) {
        ...
        if (this.descriptorObj && Object.keys(this.descriptorObj).length > 0) {
