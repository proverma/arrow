...
function SessionFactory(config, args) {
    ...
    this.proxyLogHandler = undefined;
...
SessionFactory.prototype.tearDown = function (testQueue, wdSessions) {
    ...
    if (this.startProxy) {
        ...
        this.logger.debug("closing proxy log");
        try {
            if (this.proxyLogHandler) {
                fs.closeSync(this.proxyLogHandler);
            }
        }
        catch(e) {
            this.logger.error("Error in closing proxy stream" + e);
        }
