...
setSSL = function (descriptorJson) {
    this.testJson = descriptorJson;
    strJson = JSON.stringify(this.testJson);
    strFinalJson = strJson.replace(/http:/g,"https:");
...
//        this.descLib = "";
        ...
        if (commonLib) {
            console.log('\n***Getting all tests - ' + relativePath);
            descLib = this.libManager.getAllTest(commonLib, relativePath);
            console.log('\n***Desc lib - ' + JSON.stringify(descLib));
        }
        ...
        for (testName in dp) {
            ...
            console.log('*****Testdata 1 for ' + testName + ' ::' + JSON.stringify(testData));
            ...
            console.log('*****Testdata 2 for ' + testName + ' ::' + JSON.stringify(testData))
            console.log('*****Testdata 3 relative path for ' + testName + ' ::' + testData.relativePath);

            // Store relativePath of the json file, if not already set [ Will be set already in case of imported descriptor ]
//            console.log('***Testdata..relativePath::' + testData.relativePath);
...
//            }
            ...
            console.log('\n******Testdata XXXX::' + JSON.stringify(testData) + '\n\n');
            tests.push(testData);
...
SessionFactory.prototype.tearDown = function (testQueue, wdSessions) {
    ...
    if (this.config.startPhantomJs === true) {
