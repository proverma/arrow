...
function SessionFactory(config, args) {
    ...
    this.descriptorMap = new Object();

    this.testSessionMap = new Object();

    if (args.reportFolder){
        console.log('****Report folder is passed...' + args.reportFolder);
        this.reportFolder = args.reportFolder;
    }else{
        this.reportFolder = 'arrow-target/reports';
        console.log('****Report folder instantiated to...' + this.reportFolder);
    }
...
//            console.log('\n\n****-SessionFactory runTest callback..finished ' + testSession.args.testName + ' from descriptor:' + testSession.descriptor);
            //TODO - Pranav - If all testsessions in a descriptor are finished, log a message to console
            for (var key in self.descriptorMap){
                if (key === testSession.descriptor){

                    console.log('****-Pushing testSession in ');

                    // Store testSessions array for each descriptor in testSessionMap
                    if (self.testSessionMap[key]){
                        testSessionsArr = self.testSessionMap[key];
                    }else{
                        testSessionsArr = new Array();
                    }
                    testSessionsArr.push(testSession);
                    self.testSessionMap[key] = testSessionsArr;

                    console.log('****-Key matches with ..' + testSession.descriptor+ ' ,decrementing test count..');
                    self.descriptorMap[key] = self.descriptorMap[key] - 1;

                    if (self.descriptorMap[key] === 0){
                        self.logger.info('****-All tests over for descriptor:' + testSession.descriptor);
                        console.log('****-All tests over for descriptor:' + testSession.descriptor +
                         ...
                         // TODO - Check for anything wrong in reportObj or missing or extraneous information
                            ...
                            var reportObj = {
                                ...
                                "arrTestSessions" : self.testSessionMap[key],
                                "descriptor" : testSession.descriptor,
                            ...
                            var reportManager = new ReportManager(reportObj);
                           var reportFile = reportManager.writeReports();
                            console.log('****\n\n\n');
            ...
            // and write report per descriptor
...
SessionFactory.prototype.tearDown = function (testQueue, wdSessions) {
    ...
    console.log("****-SessionFactory:TearDown: Descriptor-" + this.descriptor);
