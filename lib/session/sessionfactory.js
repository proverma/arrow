...
function SessionFactory(config, args) {
    ...
    this.descriptor = args.descriptor; // TODO - Is this required ?? Remove ,if not
    ...
    // Map ( key,value) --> (descriptor file, [list of tests in the descriptor]
    this.testMap = new Object();
...
SessionFactory.prototype.runAllTestSessions = function () {
    ...
    function startTest() {
        ...
        if (self.arrDescriptor) {
            ...
            var testKeys = Object.keys(self.testMap);
            console.log('TestMap testKeys::' + testKeys);

//            for(var j = 0; j < testKeys.length; j++){
//                var testObj = self.testMap[testKeys[j]];
//                console.log('For ' + testKeys[j] + ' ::' + JSON.stringify(testObj));
//            }
...
SessionFactory.prototype.getFactoryTests = function () {
    ...
    console.log('ArrDescriptor length:::' + this.arrDescriptor.length);
    for (i = 0; i < this.arrDescriptor.length; i += 1) {
        ...
        console.log('TestJson for -: '  + descFile + ' :' + JSON.stringify(testJson));
        console.log('\n\n');
        ...
        console.log('TestJson for -: '  + descFile + ',  dp :' + JSON.stringify(dp));
        console.log('\n\n');
        ...
        for (testName in dp) {
            //TODO - Use testName to store in map of (descriptor, testNames)
            // TODO - Test name cant be the same in one descriptor file. If it is ,duplicate tests are ignored. Expected behavior ??
            //TODO-  Shall we show error if duplicate names are used for tests within same descriptor ??
            console.log('For '+ descFile + ',test name: ' + JSON.stringify(testName));
            console.log('\n\n');
            ...
            testNameArr.push(descFile + '/' + testName); // Added to put in map
            ...
            tests.push(testData);
            console.log('Testnamearr for ' + descFile + ', ' + JSON.stringify(testNameArr));
        }

        this.testMap[descFile] = testNameArr;
        console.log('Testmap::::<<<' + this.testMap[descFile]);
    }

    return tests;
};

//TODO - To make changes to accommodate arrDescriptor changes
...
//                "descriptor" : this.arrDescriptor[i], // TODO - Not used. To remove
    ...
    //TODO - concept of child process is removed. clean up / refactor this code
