...
TestSession.prototype.setup = function (callback) {
    ...
    if (this.resolvedProxyConfigPath) {
        this.config.proxyUrl = global.routerMap[this.resolvedProxyConfigPath];
        try{
            this.config.proxyConfig = require(this.resolvedProxyConfigPath);
        }catch(e){}
    }
...
TestSession.prototype.recordFailure = function (count, callback) {
    ...
    try {
        ...
        screenShotsPath = path.resolve(global.reportFolder, "screenshots");
        if (!existsSync(screenShotsPath)) {
            fileUtil.createDirectory(screenShotsPath);
        }

        dirPath = self.descriptorPath.substring(0, self.descriptorPath.lastIndexOf("/"));
