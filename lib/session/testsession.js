...
TestSession.prototype.getArtifactPaths = function (testName, count, reportDir, screenShotsDir){
    ...
    try {
        ...
        if (global.reportFolder === '') {
            screenShotsPath = path.join(arrowTargetDir, screenShotsDir);
        } else {
        ...
        dirPath = self.descriptorPath ? self.descriptorPath : "";
        dirPath = path.resolve(screenShotsPath, dirPath);
        ...
        artifactPaths.screenShotImgPath = screenShotImgPath;
        artifactPaths.screenShotHtmlPath = screenShotHtmlPath;
...
//        console.log('\n\n');
//        console.log('**screenShotImgPath::' + artifactPaths.screenShotImgPath);
//        console.log('**screenShotHtmlPath::' + artifactPaths.screenShotHtmlPath);
...
TestSession.prototype.recordFailure = function (count, callback) {
    ...
    try {
        ...
        if (global.reportFolder === '') {
            reportDir = arrowTargetDir;
        } else {
            reportDir = global.reportFolder;
        }

        artifactPaths = self.getArtifactPaths(this.args.testName, count, reportDir, screenShotsDir);

        if (artifactPaths) {
            screenShotImgPath = artifactPaths.screenShotImgPath;
            screenShotHtmlPath = artifactPaths.screenShotHtmlPath;
        ...
        if (global.artifactsUrl) {
            ...
            pngPath = global.artifactsUrl + path.sep + path.join(screenShotsDir, self.descriptorPath, screenShotImgPath);
            pngPath = encodeURI(pngPath);

            htmlPath = global.artifactsUrl + path.sep + path.join(screenShotsDir, self.descriptorPath, screenShotHtmlPath);
            htmlPath = encodeURI(htmlPath);

        } else {
            pngPath = screenShotImgAbsPath;
            htmlPath = screenShotHtmlAbsPath;
        }
