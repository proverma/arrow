...
function TestSession(config, args, sessionId) {
    ...
    console.log('***TestSession Constructor..' + this.args.reportFolder);
...
TestSession.prototype.recordFailure = function (count, callback) {
    ...
    try {
        ...
        fileUtil.createDirectory(path.resolve(this.args.reportFolder, "screenshots"));

        if (this.args.testName) {
            testName = path.resolve(this.args.reportFolder, "screenshots", "arrow-" + this.args.testName);
        } else {
            testName = path.resolve(this.args.reportFolder, "screenshots", "arrow");
        }
