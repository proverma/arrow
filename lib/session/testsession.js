...
TestSession.prototype.retryTest = function (callback, error) {
    ...
    if (isTestPassed) {
        ...
        if (global.retryCount - self.retryCount >= 0) {
            ...
            self.recordFailure(self.retryCount, function() {
                if (isSauceLabs) {
                    ...
                    sauceLabsUtil.updateJobStatus({"passed":false, "name": self.args.testName},
                        self.driver.sauceLabsSessionId,
                        capabilities.username,
                        capabilities.accessKey,
                        function(err){
                            if (err) {
                                self.logger.debug('Error in updating sauce labs fail result status - ' + err);
                            } else {
        ...
        if (isSauceLabs) {
            ...
            sauceLabsUtil = new SauceLabsUtil();
            sauceLabsUtil.updateJobStatus({"passed":true, "name": self.args.testName},
                self.driver.sauceLabsSessionId,
                capabilities.username,
                capabilities.accessKey,
                function(err){
                    if (err) {
                        self.logger.debug('Error in updating sauce labs pass result status - ' + err);
                    } else {
