...
TestSession.prototype.retryTest = function (callback, error) {
    ...
    if (isTestPassed) {
        ...
        if (global.retryCount - self.retryCount >= 0) {
            ...
            self.recordFailure(self.retryCount, function() {
                if (isSauceLabs) {
                    ...
                    sauceLabsUtil.updateJobStatus({"passed":false, "name": self.args.testName},self.driver.sauceLabsSessionId,
                        capabilities.username,
                        capabilities.accessKey,
                        function(error){
                            if (error) {
                                self.logger.debug('Error in updating sauce labs fail result status');
                            } else {
                                self.logger.debug('Successfully updated sauce labs fail result status');
                            }
        ...
        if (isSauceLabs) {
            ...
            sauceLabsUtil = new SauceLabsUtil();
            sauceLabsUtil.updateJobStatus({"passed":true, "name": self.args.testName},self.driver.sauceLabsSessionId,
                capabilities.username,
                capabilities.accessKey,
                function(error){
                    if (error) {
                        self.logger.debug('Error in updating sauce labs pass result status');
                    } else {
                        self.logger.debug('Successfully updated sauce labs pass result status');
                    }
                    self.driver.stop(callback, error);
                });
