...
TestSession.prototype.recordFailure = function (count, callback) {
    ...
    try {
        ...
        if (global.artifactsUrl) {
            ...
            pngPath = path.resolve(dirPath, screenShotImgPath);
            htmlPath = path.resolve(dirPath, screenShotHtmlPath);
        }
        ...
        if (webdriver) {
            ...
            webdriver.takeScreenshot().then(function(img) {
                ...
                try {
                    ...
                    var paths = {};
                    if (existsSync(path.resolve(dirPath, screenShotImgPath))) {
                        paths.png = pngPath;
                    }
                }
                catch(ex) {
                    self.logger.error('Error while recording screenshot - ' + path.resolve(dirPath,screenShotImgPath));
                }
                ...
                webdriver.getPageSource().then(function(src) {
                    ...
                    try {
                        ...
                        if (existsSync(path.resolve(dirPath,screenShotHtmlPath))) {
                            paths.html = htmlPath;
                        }
                    }
                    catch(ex) {
                        self.logger.error('Error while recording HTML source - ' + path.resolve(dirPath,screenShotHtmlPath));
                    }
                    ...
                    if (paths.html) {
                        screenShotMsg += '\HTML Source: ' + paths.html + '\n';
                    } else {
                    ...
                    callback(); // TODO pass error in callback ??
                });
