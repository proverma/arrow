...
function TestSession(config, args, sessionId) {
    ...
    // Relative path of the lib wrt descriptor - Added
    //TODO - to add in config or testParams
    if (args.relativePath) {
//        this.config.relativePath = args.relativePath;
        this.testConfig.relativePath = args.relativePath;
        console.log('TestSession Constructor::' + this.testConfig.relativePath);
    }
                ...
                if (error) {
                    ...
                    arrow.runController(self.controller, self.testConfig, self.testParams, self.driver, function (error, data, controller) {
                        if (error) {
                            ...
                            console.log('In testsession, test over::' + self.args.testName);
                            if (self.isFail()) {
                                self.retryTest(callback);
                            } else {
