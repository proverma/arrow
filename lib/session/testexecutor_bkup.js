...
//            ts.logger.info('****TimeReportObj:' + JSON.stringify(sf.timeReportObj));
//            ts.logger.info('****TimeReportObj:' + JSON.stringify(sf.timeReportObj[testSession.descriptorPath].testSessionTimeArr));
            ...
            writeReportForDescriptor(function() {
                ...
                curSessionCount += 1;
//                console.log('****CurrentSessionCount:::' + curSessionCount + ' , total session count::' + totalSessionCount);
    ...
    function startTest() {
        ...
        function setupTestQueues(callback) {
            ...
            if (wdSessions.length > 0) {
                console.log('****Setting parallel to 1 since reusesession is set to true..' + wdSessions.length);
                ...
                for (i = 0; i < wdSessions.length; i += 1) {
                    ...
                    for (j = 0; j < tests.length; j += 1) {
                        ...
                        if (test.startProxyServer === true) {
                            console.log('*****Proxyserver is true for ' + test.testName + ' . So no sessions for it');
                            testQueues[i].sessions.push(new TestSession(sf.config, test));
                        } else {
                            console.log('*****Proxyserver is NOT true for ' + test.testName + ' . So sessions for it');
    ...
    if (sf.reuseSession) {
        console.log('*****In testexecutor..reuseSession set to true');
        ...
        hub.getSessions(sf, function (error, sf, arrSessions) {
            ...
            console.log('*****In testexecutor..starting tests with reusesession...' + arrSessions.length);
            startTest();
        });
    } else {
        console.log('*****In testexecutor..starting tests - no reusesession');
        startTest();
    }
