...
(function () {
    ...
    if (typeof YUI === "undefined") {
        if ((typeof process !== "undefined") && (typeof require !== "undefined")) {
            ...
            //console.log("Injecting martini seed (YUI global config)");
            var fs = require('fs');
            if (ARROW.martiniServerSeed !== undefined) {
                try {
                    if (fs.stateSync(ARROW.martiniServerSeed).isFile()) {
                        var globalConf = fs.readFileSync(ARROW.martiniServerSeed, "utf-8");
                        eval(globalConf);
                    }
                } catch (e) {
