...
ProxyManager.prototype.writeLog = function (logLine) {
    ...
    // TODO - Ideal way would be to open file once ,keep writing to it and then close it during teardown
    // Only write after 1000 lines

    if (self.logBuffer.length  === self.logLimit) {
        self.logBuffer = [];
        try {
            fd = fs.openSync(self.fileName, 'a');
            fs.writeSync(fd, self.logBuffer.toString());
            fs.closeSync(fd);
        }
        catch(e) {
            self.logger.error(' Error in writing proxy log:' + e);
        }
    }
