...
function ProxyManager(proxyConfig, config) {
    ...
    this.proxyLogFile = this.getProxyLogFileName();
}
...
ProxyManager.prototype.recordTraffic = function(router, request, response) {
    ...
    if (router[reqHost] || router[onlyhost]) {
        ...
        if ((router[reqHost] && router[reqHost].record)) {
            obj.push({"url": reqUrl, "headers": request.headers, "responseHeaders": response.headers, "method": request.method, "body": request['body']});
...
/**
 * Return the proxy log file name
 * @returns {string}
 */
ProxyManager.prototype.getProxyLogFileName = function() {

    var proxyFile = "proxy.log";
    if (global.workingDirectory) {
        proxyFile = global.workingDirectory ? path.resolve(global.workingDirectory, proxyFile) : proxyFile;
    }
    return proxyFile;
};
...
ProxyManager.prototype.runRouterProxy = function (minPort, maxPort, host, callback) {
    ...
    process.on('uncaughtException', function(err) {
        self.logger.error("Uncaught exception in ProxyManager :" + err.stack);
    });
            ...
            if (p === -1) {
                ...
                self.proxyServer = require('http').createServer(function (request, response) {
                    ...
                    if (url.parse(request.url).pathname === '/proxyCoverage') {
                        ...
                        var options, reqUrl, handle,f_http, postBodyLimit = 0;
                        options = self.getOptions(proxy_config.router || proxy_config, request);

                        reqUrl = options.headers['reqUrl'];

                        // Try to setup a buffer for POST body to store.
                        if (proxy_config && proxy_config.storeBody) {
                            postBodyLimit = proxy_config && proxy_config.sizeLimit || 10240;
                            request['body'] = {text:''};
                        }
                        ...
                        f_http.globalAgent.maxSockets = 200;
                        ...
                        proxy_request = f_http.request(options, function (proxy_response) {
                            ...
                            self.writeLog(p + ":" + reqUrl + ":" + proxy_response.statusCode);
                        ...
                        request.addListener('data', function (chunk) {
                            //Store the POST body into the body object for tracking later.
                            if (proxy_config && proxy_config.storeBody) {
                                if (postBodyLimit > request['body']['text'].length) {
                                    request['body']['text'] = (request['body']['text'] + chunk).substring(0, postBodyLimit);
                                }
                            }
...
ProxyManager.prototype.writeLog = function (logLine) {
    ...
    try {
        ...
        if (self.proxyLogFile) {
            fs.appendFileSync(self.proxyLogFile, data);
        }
    }
    catch(e){
        self.logger.error('Exception in writing proxy log ' + e);
    }
};
