...
ProxyManager.prototype.getOptions = function(router, request) {
    ...
    if (router[reqHost]) {
        this.logger.info("***In getOptions...Router reqHost :" + reqHost + ", router[reqHost]:" + router[reqHost]);
        ...
        if (router[reqHost].record) {
            this.logger.info("***In getOptions...router[reqHost].record :" + router[reqHost].record);
            self.record.push({"url": reqUrl, "headers": request.headers, "method": request.method});
        }

        if (router[reqHost].newHost) {
            this.logger.info("***In getOptions...router[reqHost].newHost :" + router[reqHost].newHost);
            reqUrl = reqUrl.replace(reqHost, router[reqHost].newHost);
            reqHost = router[reqHost].newHost;
        }
    }

    this.logger.info("***In getOptions...reqUrl :" + reqUrl);
