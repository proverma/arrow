...
ProxyManager.prototype.getOptions = function (router, request) {
    ...
    reqHost = (objUrl.host && objUrl.host.match(/:/g) ) ? objUrl.host.slice(0, objUrl.host.indexOf(":")) : objUrl.host;
    options = {
        host: reqHost,
                ...
                self.proxyServer = http.createServer(function (request, response) {
                    ...
                    if (url.parse(request.url).pathname === '/proxyCoverage') {
                        if (request.method === 'OPTIONS') {
                            ...
                            // get coverage status
                            self.logger.debug('get client sent coverage GET request from ' + request.headers['referer']);
                            if (testFinsihed) {
                                response.writeHead(200, headers);
                            } else {
                            ...
                            testFinsihed = true;  // some page send a POST then the test should be finished
                            var reqBody = '';
                        ...
                        proxy_request = http.request(options, function (proxy_response) {
                            ...
                            var isPassthroughurl = function (url) {
                                ...
                                needsRewrite = needsClientCoverage && isHtml(request, proxy_response),
                                responseStr = '',
                                buffers = [];
                                ...
                                if (needsInstrumentation || needsRewrite) {
                                    ...
                                    function doInstrument(err, code) {
                                        ...
                                        if (err) {
                                            ...
                                            self.logger.debug('Start to instrument file :' + reqUrl);
                                            codeAndPath = instrument(code.toString(), reqUrl, proxy_config);
                                            response.write(codeAndPath.code, 'binary');
                                            self.logger.debug('Instrument file :' + reqUrl + " success!");
                                        }
                                        response.end();
                                    ...
                                    function doRewrite(err, code) {
                                        ...
                                        if (err) {
                                            ...
                                            updateClientScript = function (coveragepath) {
                                                ...
                                                return clientjs.replace(/%PROXY_PATH%/g, "\'" + coveragepath + "\'");
                                            }
                                            insertMatch = /<script>|<\/head>/.exec(responseStr);
                                            insertIndex = insertMatch && insertMatch.index || 0;
                                            var coveragepath = "http://" + require('../../arrow_server/arrowservermanager').getLocalhostIPAddress() +
                                                ":" + p + "/proxyCoverage";
...
//                                            console.log(responseStr);
                                            ...
                                            self.logger.debug('Modify html file :' + reqUrl + " success!");
                                        }
                                        response.end();
