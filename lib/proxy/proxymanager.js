...
ProxyManager.prototype.getOptions = function (router, request) {
    ...
    if (router[reqHost] || router[onlyhost]) {
        ...
        if (router[reqHost].record) {
            ...
            global.record.push({"url": reqUrl, "headers": request.headers, "method": request.method});

            var obj = [];

            if (global.proxyManager.record[self.proxyConfig]) {
                obj = global.proxyManager.record[self.proxyConfig];
                obj.push({"url": reqUrl, "headers": request.headers, "method": request.method});
            ...
            if (p === -1) {
                ...
                self.proxyServer = require('http').createServer(function (request, response) {
                    ...
                    if (url.parse(request.url).pathname === '/proxyCoverage') {
                        ...
                        handle = options.headers['protocol'] === 'https:' ? require('https') : http;

                        proxy_request = handle.request(options, function (proxy_response) {
                            /*
                             if need instrument:
