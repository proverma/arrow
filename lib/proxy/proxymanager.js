        ...
        portchecker.getFirstAvailable(minPort, maxPort, host, function (p, host) {
            ...
            if (p === -1) {
                ...
                self.proxyServer = http.createServer(function (request, response) {
                    ...
                    var currentSID = global.currentSessionid;
                    if (currentSID) {
                        sessionsStatsMap[currentSID] = sessionsStatsMap[currentSID] || {};
                        sessionsStatsMap[currentSID].testFinish = sessionsStatsMap[currentSID].testFinish || false;
                    }
                    ...
                    if (url.parse(request.url).pathname === '/proxyCoverage') {
                        if (request.method === 'OPTIONS') {
                            ...
                            self.logger.debug('get client sent coverage GET request from ' + request.headers['referer'] + " with session id:" + targetSID);
                                    ...
                                    function doRewrite(err, code) {
                                        ...
                                        if (err) {
                                            ...
                                            updateClientScript = function (coveragepath) {
                                                ...
                                                return clientjs.replace(/%PROXY_PATH%/g, coveragepath).replace(/%SESSION_ID%/g, currentSID);
                                            }
