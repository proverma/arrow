...
ProxyManager.prototype.runRouterProxy = function(minPort, maxPort, host, callback) {
    ...
    this.logger.info("Starting Proxy::" + self.routerJsonPath);
    try {
        self.routerJsonPath += '/config/proxy.json';
                ...
                self.proxyServer = http.createServer(function (request, response) {
                    ...
                    proxy_request = http.request(options, function(proxy_response) {
                        ...
                        proxy_response.addListener('data', function(chunk) {
                            self.logger.info('***Writing Proxy Response::');
                        ...
                        self.logger.info("Proxy Response Headers:: "  + JSON.stringify(proxy_response.headers));
                        response.writeHead(proxy_response.statusCode, proxy_response.headers);

                    });

                    request.addListener('data', function(chunk) {
                        self.logger.info('***Writing Proxy Request::');
...
//ProxyManager.prototype.runRouterProxy2 = function(minPort, maxPort, host, callback) {
//
//    var hostName,
...
//
//            };
//            var httpServer = http.createServer(createServerCallback).listen(p);
//
//            callback(host + ":" + p);
//
//
