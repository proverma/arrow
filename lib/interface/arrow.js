...
Arrow.prototype.runController = function (controllerName, testConfig, testParams, driver, callback) {
    ...
    if (driver.getWebDriver() && driver.getWebDriver().session_) {
        driver.getWebDriver().session_.then(function (val) {
            global.currentSessionid = val.id;
        });

        //executing controller with addErrBack
        driver.getWebDriver().call(function() {
            controller.execute(controllerExecuteCallback);
        }).addErrback(function(e) {
                self.logger.debug("Exception Caught :" + e.toString());
                controllerExecuteCallback(e);
            });
    } else {
        //executing controller for non webdriver tests
        controller.execute(controllerExecuteCallback);
    }
};
