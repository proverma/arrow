...
Arrow.prototype.runController = function (controllerName, testConfig, testParams, driver, callback) {
    ...
    if (driver.webdriver) {
        driver.webdriver._listener.on("uncaughtException", function (e) {
    ...
    if (controllerName) {
        if (controllerName.indexOf(".js") !== -1) {
            ...
            //            controllerName = path.resolve(process.cwd(), controllerName);// OLD CODE
            controllerName = path.resolve(process.cwd(), testConfig.relativePath, controllerName); // ADDED
        } else {
