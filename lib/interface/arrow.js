...
Arrow.prototype.runController = function (controllerName, testConfig, testParams, driver, callback) {
    ...
    if (controllerName) {
        if (controllerName.indexOf(".js") !== -1) {
            ...
            try {
                var customControllerJson = this.config["shareLibMetaPath"] + "custom_controller.json";
                if (fs.statSync(customControllerJson).isFile()) {
                    var controllerJson = require(customControllerJson);
                    if (controllerJson[ctrName] != undefined && controllerJson[ctrName].path != undefined) {
                        controllerName = require(customControllerJson)[ctrName].path;
                    }
                }
            } catch (e) { }
        }
