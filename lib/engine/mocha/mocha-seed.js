...
mochaSeed.prototype.generateClientSideSeed = function (cb) {
    ...
    // only if user specify a reporter as html,then we need to create div for input test result
    // default we use xunit so that the test(result) wont affect the testing page.
    if(self.config.reporter == "html"){
        createDiv("mocha");
    }
