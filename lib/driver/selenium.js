                            ...
                            try {
                                var getcoverage =
                                    "if(window.frames && window.frames.length>0){ " +
                                        ...
                                        "try{ coverage.push(window.frames[i].window.__coverage__);" +
                                        "}catch(e){}" +
                                        " return coverage; }" +
                                        "}else{ return window.__coverage__; }";
                                webdriver.executeScript(getcoverage).then(function (cov) {
//                                    console.log( cov);
                                    if (Array.isArray(cov)) {
                                        for (var i = 0; i < cov.length; i++) {
                                            if (cov[i])coverage.addCoverage(cov[i]);
                                        }
                                    } else {
