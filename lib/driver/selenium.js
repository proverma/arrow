...
SeleniumDriver.prototype.createDriverJs = function (testParams, callback) {
    ...
    if (!enableYUISandbox && enableYUILoader) {
    ...
    if (testJs && (".html" === path.extname(testJs))) {
        ...
        try {
            ...
            if ((enableYUISandbox || !enableYUILoader) && depLibs && depLibs.length > 0) {
                // Remove Duplicates
    ...
    if (enableYUISandbox) {
        if (yuiDepLibs && yuiDepLibs.length > 0) {
            ...
            self.logger.info("Auto-Detected yui core modules for this test :" + yuiDepLibs);
            libscanner.scannerUtil.resolveYUIDenpendency(yuiDepLibs, {}, function (err, yuisrc) {
                if (err) {
                    yuiruntime += self.config['yuiSandboxRuntime']; // default runtime code
                    callback(err);
                } else {
                    yuiruntime += yuisrc;
                }
