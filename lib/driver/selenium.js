    ...
    if (testJs && (".html" === path.extname(testJs))) {
        ...
        try {
            ...
            if (deplibs && deplibs.length > 0) {
                // Remove Duplicates
                var hasbeenloaded = function (Libs, lib) {
                    if (!Libs || Libs.length == 0) return false;
                    for (i = 0; i < Libs.length; i++) { // (share)lib has full path,libs maybe relative path
                        try {
                            if (fs.realpathSync(Libs[i]) == fs.realpathSync(lib)) return true;
                        } catch (e) {
                            return false;
                        }
                    }
