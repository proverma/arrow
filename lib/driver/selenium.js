    ...
    if (self.config.proxyUrl) {
        ...
        if (self.config.useProxyForAll) {
            if (self.args.proxy || self.args.proxy === undefined) {
                ...
                self.logger.debug("Descriptor overridden commandline proxy param. Not setting proxy in browser");
            }
        } else {
            if (self.args.proxy) {
                self.logger.debug("Adding Proxy Setting to the Browser");
...
SeleniumDriver.prototype.start = function (callback) {
    ...
    self.wdApp = new WdAppClass();
    self.wdApp.promise.Application.getInstance().on("uncaughtException", function (e) {
    ...
    self.webdriver = new self.wdApp.Builder().
        usingServer(wdHubHost).
        usingSession(self.sessionId).
        withCapabilities(capabilities).build();
    self.webdriver.By = self.wdApp.By;
    self.webdriver.listener = self.wdApp.promise.Application.getInstance();
