...
SeleniumDriver.prototype.createDriverJs = function (testParams, callback) {
    ...
    /*
     here we should retrieve the modules that on the page but not in the sandbox
     Not clear if there're dangerous to do this.
     */
    if (enableYUISandbox) {
        testLibsJs += ";(function(){var Y=YUI();YUI.Env.mods=Y.merge(Y.config.global.YUI.Env.mods,YUI.Env.mods);}());";
    }
    ...
    if (enableYUISandbox) {
        if (yuiDepLibs && yuiDepLibs.length > 0) {
            ...
            libscanner.scannerUtil.resolveYUIDenpendency(yuiDepLibs, {version: self.config['sandboxYUIVersion']}, function (err, yuisrc, jsmods) {
                if (err) {
