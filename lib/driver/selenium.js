...
SeleniumDriver.prototype.getCapabilities = function () {
    ...
    if (self.args.proxyUrl) {
        console.log('***In selenium getCapabilities 1');

        console.log('***self.args.proxyUrl:' + self.args.proxyUrl);
        ...
        if (self.args.proxy || self.args.proxy === undefined) {
            ...
            caps.proxy = {
                ...
                "sslProxy": self.args.proxyUrl,
                "proxyType": "manual"
            };

            // If reusing session, set browser to firefox
            if (self.sessionId) {
                caps.browserName = "firefox"; // For proxy , hardcode browser to Firefox
                // TODO - Need to handle version too
            } else {
                caps.browserName = self.args["browser"] || self.config["browser"];
                if (caps.browserName === "reuse") {
    ...
    console.log('***setProxySettings:' + setProxySettings);
    console.log('***Caps 1::' + JSON.stringify(caps));
    ...
    if (setProxySettings !== true) {
        ...
        if (!caps.browserName) {
            ...
            console.log('***In selenium getCapabilities 2..' + JSON.stringify(caps));
            console.log('***Caps 2::' + JSON.stringify(caps));
            return caps;
        }
    }
    ...
    if (self.config.capabilities) {
        ...
        console.log('***In selenium getCapabilities 3..user has passed capabilities:' + JSON.stringify(self.config.capabilities));
        cm = new CapabilityManager(self.config.capabilities);
        console.log('***In selenium ..user has passed capabilities 2...' + caps.browserName);
        tmpCaps = cm.getCapability(caps.browserName);
        console.log('***In selenium ..user has passed capabilities 3');
        if (tmpCaps === null) {
            caps.error = "No related capability for " + caps.browserName + " in " + self.config.capabilities;
            console.log('***Caps 3::' + JSON.stringify(caps));
            return caps;
        }
        caps = tmpCaps;

//        if (setProxySettings === true) { // and reuse
//            caps.browserName = "firefox"; // TODO - need to handle version too ?
//        }

    } else if (setProxySettings === false) { // Browser is already set if setProxySettings is true

        // this is the case where user just provides browser as "firefox-10.0"
        browserInfo = caps.browserName.split("-", 2);
        console.log('***In selenium getCapabilities 4..user has not passed capabilities:' + JSON.stringify(browserInfo));
        ...
        console.log('***Caps 4::' + JSON.stringify(caps));

    }
    console.log('***In selenium getCapabilities 5..returning caps:' + JSON.stringify(caps));
    return caps;
...
SeleniumDriver.prototype.errorCheck = function (e, callback) {
    var self = this, errorMessage = e.toString(), match = false,
        ...
        handleArrowIsNotDefined = function (currentUrl) {
            match = matchErrorMessage(/ARROW is (not defined|undefined)/) ||
...
SeleniumDriver.prototype.start = function (callback) {
    ...
    console.log('***In selenium start 1, capabilities:' + JSON.stringify(capabilities));
    if (capabilities.error) {
        console.log('***In selenium start 2, capabilities has error' + JSON.stringify(capabilities.error));
    ...
    console.log('***In selenium start 3, wdhubhost:' + JSON.stringify(wdHubHost));
    console.log('***In selenium start 4, capabilities browsername:' + capabilities.browserName);
    ...
    if (self.sessionId) {
        console.log('***In selenium start 5, self.sessionId:' + self.sessionId);
        self.logger.info("Connecting to selenium: " + wdHubHost);
    } else {
        console.log('***In selenium start 6, self.sessionId');
