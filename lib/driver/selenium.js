...
SeleniumDriver.prototype.stop = function (callback, error) {
    ...
    if (this.sessionId) {
        ...
        if (this.webdriver) {
            self.logger.debug('Invoking webdriver quit');
...
SeleniumDriver.prototype.createDriverJs = function (testParams, callback) {
    ...
    resolvePath = function (prepath) {
        ...
        console.log('*****In resolvepath::' + prepath + ', final path::' + finalPath);
        return finalPath;
    };
    ...
    libs = testParams.lib;
    console.log('******Libs::' + JSON.stringify(lib));
    if (libs) {
        arrLib = libs.split(",");
    } else {
        arrLib = [];
    ...
    if (testJs && (".html" === path.extname(testJs))) {
        ...
        try {
            for (i = 0; i < arrLib.length; i += 1) {
                ...
                console.log('\n\n*****allDeps::' + JSON.stringify(allDeps) + '\n\n');
            ...
            console.log('\n\n****testjs in createDriverJs XX:::' + testJs + '\n\n');
            console.log('\n\n****allDeps in createDriverJs XX:::' + JSON.stringify(allDeps) + '\n\n');
            addToDependency(allDeps);

            console.log('\n\n****Deplibs in createDriverJs:::' + JSON.stringify(depLibs) + '\n\n');
            console.log('\n\n****needYUILoaderCheck:::' + needYUILoaderCheck);
