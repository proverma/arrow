...
function SeleniumDriver(config, args) {
    ...
    this.scriptTimeout = 60000;
    this.implicitWaitTimeout = 60000;
...
SeleniumDriver.prototype.start = function (callback) {
    ...
    self.webdriver.manage().timeouts().setScriptTimeout(self.scriptTimeout);
    self.webdriver.manage().timeouts().implicitlyWait(self.implicitWaitTimeout);
                ...
                hasBeenLoaded = function (arr, lib) {
                    ...
                    for (i = 0; i < arr.length; i = i + 1) { // (share)lib has full path,libs maybe relative path
                        try {
                            ...
                            if ((libname = libscanner.scannerUtil.captureYUIModuleDetails(arr[i]) != null) &&
                                (sharelibname = libscanner.scannerUtil.captureYUIModuleDetails(lib) != null) &&
                                libname.name && sharelibname.name &&
                                libname.name == sharelibname.name) {
                                return true;
                            }
                        } catch (e) {
