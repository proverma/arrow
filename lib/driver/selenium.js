...
var servermanager = require("../../arrow_server/arrowservermanager");
...
//            console.log('***Caps proxy:::' + JSON.stringify(caps.proxy));
        } else {
...
SeleniumDriver.prototype.createDriverJs = function (testParams, callback) {
    ...
    seedJs = fs.readFileSync(seed, "utf-8"); //TODO - path.resolve wrt global.workingDirectory
    runnerJs = fs.readFileSync(runner, "utf-8"); //TODO - path.resolve wrt global.workingDirectory
    libs = testParams.lib;

//    console.log('Test params lib::' + testParams.lib + '\n\n');
    if (libs) {
        arrLib = libs.split(",");
    } else {
        arrLib = [];
...
//    console.log('TestLibJS 1::' + testLibsJs + "....******\n\n");
    ...
    if (testJs && (".html" === path.extname(testJs))) {
        ...
        try {
            for (i = 0; i < arrLib.length; i += 1) {
                ...
                // get used share lib src
                if (!enableYUILoader) {
                    depLibs = depLibs.concat(libscanner.scannerUtil.getShareLibSrcByPath(lib, "client"));
                }
            }
//            console.log('TestLibJS 2::' + testLibsJs + "....******\n\n");

            if (testJs) {
                self.logger.info("Loading test: " + testJs);
//                console.log("****-Loading test: " + testJs);
                if (this.args.relativePath) {
                    testJs = path.resolve(global.workingDirectory, this.args.relativePath, testJs);
                } else {
                    testJs = path.resolve(global.workingDirectory, testJs);
                }
//                console.log("****-Loading test 1: " + testJs);
                testLibsJs += fs.readFileSync(testJs, "utf-8");
//                console.log('TestLibJS 3::' + testLibsJs + "....******\n\n");
                if (!enableYUILoader) {
                    depLibs = depLibs.concat(libscanner.scannerUtil.getShareLibSrcByPath(testJs, "client"));
                }
//                console.log("****-Loading test 2 testJs: " + testJs);
//                console.log("****-Loading test 3 testLibJs: " + testLibsJs);
            } else {
                self.logger.info("Loading action: " + actionJs + "," + this.args.relativePath);

                if (this.args.relativePath) {
                    actionJs = path.resolve(global.workingDirectory, this.args.relativePath, actionJs);
                } else {
                    actionJs = path.resolve(global.workingDirectory, actionJs);
                }

                testLibsJs += fs.readFileSync(actionJs, "utf-8");
//                console.log('TestLibJS 4::' + testLibsJs + "....******\n\n");
                if (!enableYUILoader) {
            ...
            if (depLibs && depLibs.length > 0) {
                ...
                for (i = 0; i < depLibs.length; i = i + 1) {
                    try {
                        ...
                        testLibsJs += "\n";
//                        console.log('TestLibJS 5::' + testLibsJs + '*****');
                    } catch (e) {
...
//    console.log('TestLibJS 6::' + testLibsJs + '*****');
...
//    console.log("****-Loading test 3: " + driverJs);
...
//    console.log('****In executeTest...' + testParams.lib);
    var self = this,
        logger = this.logger,
        webdriver = this.webdriver,
        testJs,
    ...
    driverJs = this.createDriverJs(testParams, callback);
//    console.log("****-In execute test 1");
    if (false === driverJs) {
        return;
    }
//    console.log("****-In execute test 2::" + driverJs);
    console.log('\n\n\n');
    if (self.minifyJS) {
...
//    console.log("****-In execute test 3:::" + driverJs);
    console.log('\n\n\n');

    function collectTestReport() {
//        console.log("****-In collectTestReport 1..");
        retryCount += 1;
        logger.debug("Waiting for the test report, attempt: " + retryCount);
//        console.log("****-Waiting for the test report, attempt: " + retryCount);
        webdriver.executeScript("return ARROW.testReport;").then(function (report) {
//            console.log("****-In collectTestReport 2..");
            if (report && report !== "{}") {
//                console.log("****-In collectTestReport 3..");
                webdriver.getCurrentUrl().then(function (u) {
                    reportJson = JSON.parse(report);
                    reportJson.currentUrl = u;
//                    console.log("****-In collectTestReport 4..");
                    self.addReport(JSON.stringify(reportJson), caps);
                });


                // This causes trouble - TODO Resolve it
//                collectTestParams(showYUIConsoleLog(function () {
//                    console.log('***In collect test params callback 1');
//                    if (self.config.coverage) {
//                        console.log('***In collect test params callback 2');
//                        try {
...
//                        }
//                    } else {
//                        console.log('***In collect test params callback 3');
//                        callback(null, report);
//                    }
//                }));
    ...
    function collectTestParams(callback) {
        try {
            console.log('***In collect test params..1');
            webdriver.executeScript("return ARROW.testParams;").then(function (params) {
                console.log('***In collect test params..2');
                if (params) {
                    console.log('***In collect test params..3');
                    self.logger.debug("Arrow.testParams:");
                    self.logger.debug(JSON.stringify(params));
                    testParams.shared = params.shared;
                } else {
                    console.log('***In collect test params..4');
                    self.logger.debug("Arrow.testParams:" + "nothing" + "\n\n");
                }
                console.log('***In collect test params..5');
                callback();
            });
        } catch (e) {
    ...
    function initTest() {
//        console.log("****-In init test 1" + JSON.stringify(driverJs) );
//        console.log("****-In init test 2");
        webdriver.executeScript(driverJs).then(function () {
//            console.log("****-In init test 3");
            setTimeout(collectTestReport, self.attemptInterval);
        });
    }

//    console.log("****-In execute test 4");
