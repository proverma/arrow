...
//    console.log('***self.config.proxyUrl:' + self.config.proxyUrl);
...
//    if (self.config.proxyUrl) {
////        console.log('***self.args.proxy:' + self.args.proxy);
//        console.log('***self.args.startProxyServer:' + self.args.startProxyServer);
//        // Either of startProxyServer or proxy are defined
//        if (self.args.startProxyServer !== undefined || self.args.proxy !== undefined) {
//            console.log('***Here....');
//            // Only pass proxy capabilities if startProxyServer is NOT set to false
//            if (self.args.startProxyServer !== false){
//
//                // Only pass proxy capabilities if proxy is NOT set to false
//                if (self.args.proxy !== false) {
//                    self.logger.debug("Adding Proxy Setting to the Browser");
//                    caps.proxy = {
//                        "httpProxy": self.config.proxyUrl,
//                        "sslProxy": self.config.proxyUrl,
//                        "proxyType": "manual"
//                    };
//                } else {
//                    self.logger.debug("Descriptor overridden proxy param. Not setting proxy in browser");
//                }
//            } else {
//                self.logger.debug("startProxyServer is set to false.Not setting proxy in browser");
//            }
//        }
//    }
...
//
//        if (self.args.startProxyServer === true || self.args.startProxyServer === true) {
//            if (self.args.proxy === true || self.args.proxy === undefined) {
//                self.logger.debug("Adding Proxy Setting to the Browser");
//                caps.proxy = {
//                    "httpProxy": self.config.proxyUrl,
//                    "sslProxy": self.config.proxyUrl,
    ...
    if (testJs && (".html" === path.extname(testJs))) {
        ...
        try {
            ...
            if (!needYUILoaderCheck && depLibs && depLibs.length > 0) {
                ...
                self.logger.info("After Auto-Detected libs to load for this test :" + depLibs);
            }
...
SeleniumDriver.prototype.executeAction = function (testConfig, testParams, callback) {
    ...
    function initAction() {
        self.logger.debug("In initAction..");
        webdriver.executeScript(driverJs).then(function () {
            self.logger.debug("In initAction..Script executed..");
            setTimeout(collectActionReport, self.attemptInterval);
        });
    }
    ...
    this.createDriverJs(testParams, function (err, driverjs) {
        if (err) {
            self.logger.debug("Error - Callback of createDriverjs.." + err);
            return callback(err);
        }
        driverJs = driverjs;
        self.logger.info("Now loading page::" + page);
        if (page) {
            self.logger.info("Loading page: " + page);
            ...
            self.logger.info("Calling initAction...");
            initAction();
        }
    });
};
...
SeleniumDriver.prototype.executeTest = function (testConfig, testParams, callback, web_driver) {
    ...
    function initTest() {
//        console.log('****In inittest1..' + driverJs);
        webdriver.executeScript(driverJs).then(function () {
//            console.log('****In callback of executescript inittest1..');
            setTimeout(collectTestReport, self.attemptInterval);
        });
    }

    this.createDriverJs(testParams, function (err, driverjs) {
        self.logger.debug("Callback of createDriverJs 1::" + err);
        if (err) {
            return callback(err);
        }
        driverJs = driverjs;
        ...
        if (page && testParams.customController === false) {
            ...
            self.logger.debug("Calling initTest 1");
            initTest();
        }
    });
};
