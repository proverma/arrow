...
//                "httpProxy": global.routerMap[self.resolvedRouterConfigPath],
            ...
            self.logger.info('Caps proxy:::' + JSON.stringify(caps.proxy));
...
SeleniumDriver.prototype.executeAction = function (testConfig, testParams, callback) {
    ...
    logger.info('***In executeAction..testConfig:' + JSON.stringify(testConfig) + ',' + 'testParams:' + JSON.stringify(testParams));
...
SeleniumDriver.prototype.executeTest = function (testConfig, testParams, callback) {
    ...
    logger.info('***In executeTest..testConfig:' + JSON.stringify(testConfig) + ',' + 'testParams:' + JSON.stringify(testParams));
    ...
    function collectTestParams(callback) {
        try {
            webdriver.executeScript("return ARROW.testParams;").then(function (params) {
                if (Object.keys(params.shared).length) {
