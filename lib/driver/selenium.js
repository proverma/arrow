...
//    delete require.cache[require.resolve('../../ext-lib/webdriver/_base.js')]; TODO - If this is uncommented , only one descriptor runs
...
SeleniumDriver.prototype.createDriverJs = function (testParams, callback) {
    ...
    if (testJs && (".html" === path.extname(testJs))) {
        ...
        try {
            ...
            if (testJs) {
                self.logger.info("Loading test: " + testJs + " ,Global working dir::" + global.workingDirectory + ",Relative path::" + this.args.relativePath );
                console.log('Resolved path::' + path.resolve(global.workingDirectory, this.args.relativePath, testJs));

                testJsAbsPath = path.resolve(global.workingDirectory, this.args.relativePath, testJs);
                //testLibsJs += fs.readFileSync(testJs, "utf-8"); OLD CODE TODO Remove this comment
                testLibsJs += fs.readFileSync(testJsAbsPath, "utf-8"); // Added
                if (!enableYUILoader) {
                ...
                actionJsAbsPath = path.resolve(global.workingDirectory, this.args.relativePath, actionJs);
                console.log('ActionJsAbspath::' + actionJsAbsPath);
                testLibsJs += fs.readFileSync(actionJsAbsPath, "utf-8");
// testLibsJs += fs.readFileSync(actionJs, "utf-8"); // Old code
                if (!enableYUILoader) {
