...
SeleniumDriver.prototype.createDriverJs = function (testParams, callback) {
    ...
    if (!testJs) {
        return callback("The test js must be specified");
    }
 ...
 */
SeleniumDriver.prototype.navigate = function (testParams, callback) {
    var self = this,
        logger = this.logger,
    ...
    if (false === url) {
        ...
        self.resizeWindow(testParams.windowSize);
        ...
        webdriver.executeScript("return ARROW.testReport;").then(function (report) {
                if (report && report !== "{}") {
                    webdriver.getCurrentUrl().then(function (u) {
                        ...
                        if (self.sauceLabsSessionId) {
                            reportJson.sauceLabsReportUrl = 'http://saucelabs.com/jobs/' +
                                self.sauceLabsSessionId;
                        }
    ...
    this.createDriverJs(testParams, function (err, driverjs) {
        ...
        webdriver.getCapabilities().then(function (val) {
            ...
            if (self.config.isSauceLabs) {
                if (val.caps_ && val.caps_['webdriver.remote.sessionid']) {
                    logger.debug('Got sauce labs session id');
                    self.sauceLabsSessionId = val.caps_['webdriver.remote.sessionid'];
                }
            }
            caps = val;
        });
...
SeleniumDriver.prototype.resizeWindow = function(windowSize) {
    if (!this.isMobile) {
        if (windowSize) {
            this.webdriver.manage().window().setSize(parseInt(windowSize.width, 10), parseInt(windowSize.height, 10));
            this.webdriver.manage().window().setPosition(0, 0);
        } else {
