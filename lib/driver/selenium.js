...
SeleniumDriver.prototype.createDriverJs = function (testParams, callback) {
    ...
    enableYUISandbox = self.config["useYUISandbox"] || false;
    ...
    if (enableYUISandbox) {
        if (yuiDepLibs && yuiDepLibs.length > 0) {
            ...
            libscanner.scannerUtil.resolveYUIDenpendency(yuiDepLibs, {root: self.config['sandboxYUIVersion'] + "/build/"}, function (err, yuisrc) {
                if (err) {
