...
NodeDriver.prototype.createNodeArgs = function (testParams, callback) {
    ...
    libs = testParams.lib;
    martiniServerSeed = require('path').join(self.config["arrowModuleRoot"] ,"tmp", self.config["serverConfigName"]);
    self.logger.debug("martini seed: " + martiniServerSeed);

    nodeArgs = {
        "seed": seed,
        "martiniServerSeed": martiniServerSeed,
        "runner": runner,
        "test" : testJs,
        "libs": libs,
