...
ReportManager.prototype.getScreenShotsMsg = function(screenShotPaths) {
    ...
    if (screenShotPaths && screenShotPaths.length > 0) {
        ...
        screenShotMsg = '\n\nPlease find the artifacts here:\n';
        for (var i = 0; i < screenShotPaths.length; i+=1) {
            var msg = ' (';
            msg += (i+1);
            msg += ')\n';
            msg += ('Screenshot: ' + screenShotPaths[i].png + '\n');
            msg += ('HTML: ' + screenShotPaths[i].html + '\n');
            screenShotMsg += msg;
        }
    }
        ...
        if (!resultPresent) {
            try {
                ...
                if (errorRep) {
                    ...
                    var screenShotMsg = this.getScreenShotsMsg(testSessions[i].screenShotPaths);
