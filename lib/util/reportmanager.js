                ...
                for (m in testJson[l]) {
                    if (testJson[l][m].type === "test") {
                        ...
                        if (testJson[l][m].result === "fail") {
                            ...
                            if (testJson.sauceLabsReportUrl) {
                                sauceLabsReportUrl = testJson.sauceLabsReportUrl;
                                sauceLabsReportMsg = '\n\n\nSauce Labs Report : ' + sauceLabsReportUrl;
                            }

                            self.addTest(timePerTest, className, testName, testJson[l][m].message +
                                "." + "\n" + " URL :" +
                                currentUrl + sauceLabsReportMsg + screenShotPathsMsg);
...
ReportManager.prototype.showReportOnConsole = function (result, verbose) {
    ...
    if (result.sauceLabsReportUrl) {
        log("Sauce labs report: " + result.sauceLabsReportUrl);
        log("");
    }
...
ReportManager.prototype.writeReports = function () {
    ...
    if (this.reportObj.descriptor) {
        ...
        log(("Test Report Summary for the descriptor::" + this.reportObj.descriptor + ' ' +  dataDriverKey).bold.magenta.toString());
