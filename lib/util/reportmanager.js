...
ReportManager.prototype.writeReports = function () {
    ...
    if (this.reportObj.descriptor) {
        ...
        console.log('****this.reportObj.descriptor::' + this.reportObj.descriptor);

//        reportFilePath = path.basename(this.reportObj.descriptor);
        reportFilePath = this.reportObj.descriptor;
...
//        jsonReportName = path.basename(reportFilePath, '.json') + "-report.json";
        dirPath = reportFilePath.substring(0, reportFilePath.lastIndexOf("/"))  + "/";
//        console.log('****dirPath::' + dirPath);

        if (global.reportFolder) {
            dirPath = path.resolve(global.reportFolder, 'arrow-report', dirPath);
//            console.log('****After resolving dirpath::' + dirPath);

            // Create directory if it doesnt exist
            // This check helps when keepTestReport is set to true ( If its not set to true, arrow-report would have been cleaned up anyways
            if (!existsSync(dirPath)) {
//                console.log('****Creating directory::' + dirPath);
                fileUtil.createDirectory(dirPath);
            } else {
//                console.log('***The directory ' + dirPath + ' already exists');
            }

        }

        xmlReportName = path.resolve(dirPath, path.basename(reportFilePath, '.json') + "-report.xml");
        jsonReportName = path.resolve(dirPath, path.basename(reportFilePath, '.json') + "-report.json");
//        jsonReportName = path.basename(reportFilePath, '.json') + "-report.json";
