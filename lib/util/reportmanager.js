...
ReportManager.prototype.writeReports = function () {
    ...
    if (global.reportFolder) {
        ...
        for (i = 0; i < testSessions.length; i = i + 1) {
            ...
            // TODO - To solve json report issue
            //            var reports = testSessions[i].driver.getReports();
            //            console.log('\n\n***Reports::' + JSON.stringify(reports));
            //            jsonReport[testSessions[i].args.browser + "." + testSessions[i].driver.testName] = JSON.parse(JSON.stringify(reports));
            //            var ua = UserAgent.parse(reports.results[0].ua).toAgent();
            //            jsonReport[ua + "." + testSessions[i].driver.testName] = JSON.parse(JSON.stringify(reports));
