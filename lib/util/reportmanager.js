...
//        console.log('***Testsessions ::' + JSON.stringify(testSessions[i]));
        ...
        if (!resultPresent) {
            ...
            try {
                if (errorRep.results && errorRep.results.length > 0) {
                    ...
                    this.addTest(0, "Error." + testSessions[i].driver.testName, "Error", "Arrow didn't receive result from the test !!!!");
                }
            } catch (e) {
                self.logger.error(e);
            }
    ...
    if (this.reportObj.descriptor) {
        ...
        dirPath = this.reportObj.descriptor.substring(0, this.reportObj.descriptor.lastIndexOf("/"));
