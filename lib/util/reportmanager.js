...
function ReportManager(reportObj) {
    ...
    if (reportObj){
        console.log('****In ReportManager constructor-' + reportObj.descriptor);
    }
...
ReportManager.prototype.writeReports = function () {
    ...
    for (i = 0; i < testSessions.length; i = i + 1) {
        ...
        console.log('\n\n\n****-In reportmanager..' + i + ', ' + JSON.stringify(rep));
        ...
        for (j = 0; j < arrReport.length; j = j + 1) {
            if (rep.scenario) {
                ...
                for (k = 0; k < results.length; k = k + 1) {
                    ...
                    testJson =  results[k];
                    console.log('****-In reportmanager..results -0-' + JSON.stringify(testJson));
                ...
                console.log('****-In reportmanager..results -1- ' + JSON.stringify(testJson));
                if (testJson.type === "report") {
                    resultPresent = true;
                    this.parseYUIResults(testJson, testSessions[i].driver.testName, this);
                }
    ...
    // TODO - If testsessions are from one descriptor, use descriptor name in the report file name
    //TODO - Pranav - Review this logic
    if (this.reportObj.descriptor){
        console.log('****ReportObj descriptor::' + this.reportObj.descriptor);
        reportFilePath = this.reportObj.descriptor.substring(this.reportObj.descriptor.lastIndexOf(path.sep)+1);
        xmlReportName = reportFilePath.split(".")[0] + "-report.xml";
        jsonReportName = reportFilePath.split(".")[0] + "-report.json";
    }else{
        // For consolidated report , use this
        console.log('****For consolidated report::');
        xmlReportName = "report.xml";
        jsonReportName = "report.json";
    }

    console.log('****-xmlReportName::' + xmlReportName);
    console.log('****-jsonReportName::' + jsonReportName);
    console.log('****-Report folder::' + self.reportFolder);

    // Store report files in user specified location, if explicitly specified, otherwise store in arrow-target/reports
    //TODO - If condition is not required as self.reportFolder should never be undefined. - Pranav, Remove if condition
    if (self.reportFolder) {
        xmlReportName = self.reportFolder  +  xmlReportName;
        jsonReportName = self.reportFolder  +  jsonReportName;
    }
    else{
        // TODO - Pranav - Remove else block
        console.log('****-Control should NOT go here..');
        reportDirPath = global.workingDirectory + "/arrow-target/reports"; //TODO - Pranav - Reports should go under "arrow-target/reports" directory
        console.log('ReportDirPath::' + reportDirPath);

        if(! fs.existsSync(reportDirPath)){
            fs.mkdirSync(reportDirPath);
        }else{
            stats = fs.lstatSync(reportDirPath);
            if (! stats.isDirectory(reportDirPath)){
                fs.mkdirSync(reportDirPath);
            }else{
            }
        }

        xmlReportName = reportDirPath + path.sep + xmlReportName;
        jsonReportName = reportDirPath + path.sep + jsonReportName
    }
