...
DataProvider.prototype.getTestData = function () {
    ...
    isDataDriven = descriptorJson[0].dataDriver || (descriptorJson[0].config && descriptorJson[0].config.length > 0);

    // Process data driven descriptors
    if (isDataDriven) {

        dataDrivenDescriptorArr = dataDriverManager.processDataDriver(relativePath, descriptorJson);
