...
DataProvider.prototype.getTestData = function () {
    ...
    descriptorJson = JSON.parse(descriptorJsonStr); // TODO Error checking
    relativePath = path.dirname(self.testDataPath);
    dataDriverPath = descriptorJson[0].dataDriver;

    // Process data driven descriptors
    if (dataDriverPath) {

        dataDriverPath = path.resolve(global.workingDirectory,relativePath, dataDriverPath);

        dataDrivenDescriptorArr = dataDriverManager.processDataDriver(dataDriverPath, descriptorJson);

        if (dataDrivenDescriptorArr && dataDrivenDescriptorArr.length > 0) {
            for (var i = 0; i < dataDrivenDescriptorArr.length; i+=1) {
                var descJson = dataDrivenDescriptorArr[i];
