...
DataProvider.prototype.getTestData = function () {
    ...
    if (this.defaultParamJSON || this.replaceParamJSON) {
        ...
        if (this.defaultParamJSON) {
            ...
            if (this.defaultParamJSON.indexOf(".json") > 0) {
                defaultParamPath = path.resolve(global.workingDirectory, this.defaultParamJSON);
                defaultParam = fs.readFileSync(defaultParamPath, "utf-8");
            } else {
                defaultParam = this.defaultParamJSON;
            }

            try {
                defaultParamJson = JSON.parse(defaultParam);
            } catch (defaultParamMsg) {
                this.logger.error(defaultParam + " is not a valid JSON");
                this.logger.error(defaultParamMsg);
                process.exit(1);
            }
        ...
        if (this.replaceParamJSON) {
            ...
            } catch (replaceParamMsg) {
                this.logger.error(replaceParam + " is not a valid JSON");
                this.logger.error(replaceParamMsg);
                process.exit(1);
            }
