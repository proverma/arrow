...
ArrowSetup.prototype.setupReportDir = function () {
    ...
    if (this.argv.reportFolder){
        ...
        // Create directory
        fileUtil.createDirectory(path.resolve(global.workingDirectory, this.argv.reportFolder));

    }else{
        console.log('****-Report folder NOT passed by the user so creating default arrow-target directory');
        this.argv.reportFolder = 'arrow-target' + path.sep + 'reports' + path.sep;
        ...
        fileUtil.removeDirectory(path.resolve(global.workingDirectory,'arrow-target'));
