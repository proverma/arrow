...
//        self.logger.info('***ReportFolder argv::' + this.argv.reportFolder);

        // If reportFolder is passed in the argument,
        if (this.argv.reportFolder) {
            targetFolderRelPath = path.join(this.argv.reportFolder,arrowTargetDir);
            targetFolderPath =  path.resolve(global.workingDirectory, this.argv.reportFolder, arrowTargetDir);
        } else {
            // Report folder not passed by the user
            // By default, reports shall go under arrow-target/reports
            targetFolderPath =  path.resolve(global.workingDirectory, arrowTargetDir);
            targetFolderRelPath =  arrowTargetDir;
        }
...
//        console.log('***ReportFolderPath::' + reportFolderPath);

        fileUtil.createDirectory(reportFolderPath);

        if (artifactsUrl) {
            console.log('**Argv artifactsUrl 1::' + artifactsUrl);
            console.log('**targetFolderRelPath 1::' + targetFolderRelPath);

//            artifactsUrl = artifactsUrl + path.sep + targetFolderRelPath;
            artifactsUrl = path.join(artifactsUrl , targetFolderRelPath);
            console.log('**Argv artifactsUrl 2::' + artifactsUrl);
            self.logger.info('Global Artifacts Url:' + artifactsUrl); // TODO - Remove this comment
        }
        ...
        if (artifactsUrl) {
            self.logger.info('Rel path for arrow-target:' + path.join(arrowTargetDir));
            console.log('**Argv artifactsUrl 1::' + artifactsUrl);
            artifactsUrl = path.join(artifactsUrl , arrowTargetDir);
            console.log('**Argv artifactsUrl 2::' + artifactsUrl);
            self.logger.info('Global Artifacts Url:' + artifactsUrl); // TODO - Remove this comment
        }
        global.artifactsUrl = artifactsUrl;
