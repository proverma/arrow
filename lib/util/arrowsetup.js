...
//TODO - Pranav - Get rid of deleteOldFiles
...
ArrowSetup.prototype.setupReportDir = function () {
    ...
    console.log('****Arrowsetup reportFolder:' + this.argv.reportFolder);

    // If reportFolder is passed in the argument,
    if (this.argv.reportFolder) {
        console.log('****-Report folder passed by the user -' + this.argv.reportFolder);
        reportFolderPath =  path.resolve(global.workingDirectory, this.argv.reportFolder);
    } else {
        // Report folder not passed by the user
        // By default, reports shall go under arrow-target/reports
        console.log('****-Report folder NOT passed by the user so creating default arrow-target directory');
        reportFolderPath =  path.resolve(global.workingDirectory, 'arrow-target');
    }

    reportFolderPath = reportFolderPath + path.sep + 'arrow-report';
    console.log('****-In setup, removing directory...');
    // Clean up report directory ,if it already exists
    fileUtil.removeDirectory(reportFolderPath);

    console.log('****-In setup, creating directory...');
    // Create directory to store reports
    fileUtil.createDirectory(reportFolderPath);

    this.argv.reportFolder = reportFolderPath;
...
ArrowSetup.prototype.setupHeadlessParam = function () {
    ...
    if (".json" === ext) {
        // Store .json files in arrDescriptor - Added Pranav
