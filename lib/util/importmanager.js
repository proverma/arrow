    ...
    try {
        ...
        if (importedDescriptorArr) {
            ...
            for (var i = 0; i < importedDescriptorArr.length; i+=1) {
                ...
                self.logger.debug('***Relative path:' + importingDescRelPath);
                ...
                self.logger.debug('***importedDescriptorArr[i].path:' + importedDescriptorArr[i].path);
                importedDescPath = path.dirname(path.join(importingDescRelPath, importedDescriptorArr[i].path));
                self.logger.debug('***importedDescPath Dirname:' + importedDescPath);
 ...
 * @returns {boolean}
 */
ImportManager.prototype.addTest = function(importTest,importGroup,importJson,key){
