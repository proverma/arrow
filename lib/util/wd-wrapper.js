...
wd.WebDriver.prototype.waitForElementPresent = function(elm,timeout) {
    ...
    var wdSelf = this;
    return this.wait(function() { self.logger.info("Waiting for Element: " + elm);return wdSelf.isElementPresent(elm)},timeout, 'Test timed out while waiting for ' + elm.value);
};
...
wd.WebDriver.prototype.waitForNextPage = function(currentUrl, timeout){
    ...
    return this.wait(function() {
            if (nextPageUrl.length > 0) {
                ...
                self.logger.info("Next page: " + this._currentUrl);
                return true;
            }
