...
function getArrowServerIPAddr() {
    ...
    var ip;
    if (fs.existsSync(statusfile) && fs.statSync(statusfile).isFile()) {
...
function doScan() {
    ...
    if (!fs.existsSync(pathForScan) || !fs.statSync(pathForScan).isDirectory()) {
...
function isIgnoreScanDir(f) {
    return arrowConfig.ignoreScan.contains(f)
        || !f.toString().indexOf("martini") == 0;
}
...
exports.initSeed=function() {
    ...
    scan(pathForScan, function (err, message) {
        if (err) {
            ...
            console.log(message+" Total time :"+(new Date().getTime()-start)/1000 +" s");
            //write back modules info
            writeSeedFile(function (er, message) {
                if (er) {
                    console.log(err);
                } else {
                    console.log(message);
...
function  writeSeedFile(cb){
    ...
    fs.writeFile(path.join(shareLibMetaPath, arrowConfig.clientConfigName), client_template.supplant(config), function (err) {
        if (err) cb(err);
        fs.writeFile(path.join(shareLibMetaPath, arrowConfig.serverConfigName), server_template.supplant(config), function (err) {
            if (err) cb(err);
            else cb(null, "Write modules meta info to " + path.normalize(shareLibMetaPath));
        })
    });
}
function prepareScan(libname) {
    ...
    var GROUP_BASE = arrow_server_ipaddr || "localhost:10000";  //to be replace if arrow server not started

    config_client[libname] = {};
    config_client[libname].base = GROUP_BASE;
    config_client[libname].root = GROUP_ROOT;
    config_client[libname].modules = {};

    // set up for server side base and root
    GROUP_BASE = scanlibpath;
