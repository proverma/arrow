...
function ArrowRecursive(config, args) {
    ...
    if (this.args.capabilities) {
        for (k in this.recArgs) {
            if (this.recArgs[k].indexOf("capabilities") !== -1) {
                this.recArgs[k] = "--capabilities=" + path.resolve(global.workingDirectory, this.args.capabilities);
            }
        }
    }
    // set up engine config,it could be fs path or string ,has been parsed in main-set up
