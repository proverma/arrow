...
function writeSeedFile(cb) {
    ...
    if(Object.keys(config_client).length !== 0)removeEmptyModulesOfConfig(config_client);
    if(Object.keys(config_server).length !== 0)removeEmptyModulesOfConfig(config_server);
            ...
            fs.stat(file, function (err, stat) {   // enable soft link
                if (stat && stat.isDirectory() && !isHiddenFile(f)) {
                    if (!isShareLib(file)) { // recursive scan or not by config
                        ...
                        // first parames is lib name like martini_A ,
                        // second parames is lib path like path/to/martini_A
