...
function writeSeedFile() {
    ...
    try {
		...
		shareLibLogger.debug("Write sharelib modules meta done!");
...
function captureYUIModuleDetails(filePath) {
    ...
    try {
        ...
        yui = null;
		shareLibLogger.debug('Failed to parse javascript(YUI) file: ' + filePath + '\n' + e.message, 'error');
    }
    return yui;
}
...
function getShareLibSourcePath(libname, affnity) {
    ...
    var server_config = {},
        client_config = {};
    try {
        server_config = require(path.join(shareLibMetaPath, SERVER_CONFIG_NAME));
    } catch (e) {
    ...
    try {
        client_config = require(path.join(shareLibMetaPath, CLIENT_CONFIG_NAME));
