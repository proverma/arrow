...
var shareLibMetaPath = (function () {
    ...
    console.log('\n\n*****In shareLibMetaPath, Metapath :: ' + metapath);
    return metapath;
})();
...
function scan(scanlibpath, cb) {
    ...
    console.log('***In scan , scanlibpath ::' + scanlibpath);
...
function getShareLibSourcePath(libname, affnity) {
    ...
    var server_config = {},
        client_config = {},
        foundLibs = [];
    console.log('**In getShareLibSourcePath 1 - libname :: ' + JSON.stringify(libname));
    try {
    ...
    console.log('**In getShareLibSourcePath 2 - libname , Server config ::' + server_config + ' , Client config ::' + client_config);
    function findSrcFromSeed(config, libname, affnity) {
        ...
        (function findLibInConfig(config, libname, affnity) {
            ...
            console.log('****In findLibInConfig 1 :: ' + libname);

            var lib;
            for (var key in config) {
                console.log('****In findLibInConfig 2 :: ' + key);

                if (config.hasOwnProperty(key) &&
                    config[key]['modules'] &&
                    config[key]['modules'][libname]) { //find libname

                    console.log('****In findLibInConfig 3 has key :: ' + key);

                    // if dumplicated,then dont add to libs
                    if (contains(libs, config[key]['modules'][libname].path)) continue;

                    console.log('****Pushing ' + config[key]['modules'][libname].path);
                    libs.push(config[key]['modules'][libname].path);

                    if (config[key]['modules'][libname].requires) {
                        console.log('****In findLibInConfig 3 requires :: ' );
...
scannerUtil.getSrcDependencyByPath = function getSrcDependencyByPath(libpath, affnity) {
    console.log('**In getSrcDependencyByPath 1 :: ' + libpath);
    var allDeps = {},
        shareLibsJs = [],
        yreq = captureMutipleYUIModuleDetails(libpath),
        i,
        j;
    console.log('**In getSrcDependencyByPath 2 - yreq :: ' + JSON.stringify(yreq));
    /*
     NOTES:
    ...
    console.log('**In getSrcDependencyByPath 3 - shareLibsJs :: ' + JSON.stringify(shareLibsJs));
    ...
    if (shareLibsJs && shareLibsJs.length > 0) {
        ...
        for (i = 0; i < shareLibsJs.length; i++) {
            ...
            console.log('**For ' + shareLibsJs[i] + ',  - yreq is :: ' + JSON.stringify(yreq));
            if (yreq && yreq.length > 0) {
