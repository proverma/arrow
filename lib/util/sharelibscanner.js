...
// ignore modules(folder) does not start with "martini_" or "dev_"

function isIgnoreScanDir(f) {

    for (var i = 0; i < SCAN_MODULES_PREFIX.length; i++) {
        if (f.toString().indexOf(SCAN_MODULES_PREFIX[i]) == 0) return false;
    }
    return true;
}
        ...
        list.forEach(function (f) {
            ...
            fs.stat(file, function (err, stat) {
                if (stat && stat.isDirectory()) {
                    ...
                    // libname like martini_A , libpath=path/to/martini_A
