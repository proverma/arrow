...
var SHARE_LIB_SCAN_RECURSIVE = arrowConfig.scanModulesRecursive || true;
 ...
 * that 's modules(folder) start with "martini_" or "dev_" or is just build-in lib : arrow/sharelib/
...
/**
 * generate meta data for a martini_/dev_ packages or build-in arrow/sharelib
 * @param libname
 * @param libpath
 * @param done
 */
            ...
            fs.stat(file, function (err, stat) {
                if (stat && stat.isDirectory() && !isHiddenFile(f)) {
                    if (isShareLibYUILib(f)) {  // should be "lib" foder
                        ...
                        // for some folders like "node_modules" or others , scan recursive
                        scan(file, function (err, message) {
                            if (err) return done(err);
                            if (!--pending) {
                                return done(null, "-- Processing modules " + libname + " done!");
...
/**
 * generate meta data for a martini_/dev_ modules or build-in sharelib
 * @param libname
 * @param libpath
 * @param done
 */
function generateYUIGroupsInfo(libhash, libpath, done) {
    ...
    fs.readdir(libpath, function (err, list) {
        ...
        list.forEach(function (f) {
            ...
            fs.stat(file, function (err, stat) {
                if (stat && stat.isDirectory() && !isHiddenFile(f)) {
                    if (isShareLibYUIModule(f)) { // should be "server","common",or "client" folder
...
/**
 * generate meta data of controller under a martini_/dev_ pkg or build-in sharelib
