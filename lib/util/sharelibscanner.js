...
scannerUtil.resolveYUIDenpendency = function resolveYUIDenpendency(yuideps, opts, cb) {
    ...
    var Y = require('yui').YUI(),
        ...
        loader = new Y.Loader({
            ...
            base: opts.base || path.join(ARROW_MODULES_ROOT, "node_modules", "yui"),
            combine: opts.combine || false,
            comboBase: opts.comboBase || 'http://yui.yahooapis.com/combo?',
            //Ignore things that are already loaded (in this process)
            ignoreRegistered: opts.ignoreRegistered || true,
            //And the root
            root: opts.root || "3.6.0/build/",
            //Require your deps
            require: yuideps
        });
    yuiout = loader.resolve(true);
    if (yuiout && yuiout.js) {
        var YIO = require('yui/io-base');
        sync.forEachSeries(yuiout.js, function (url, finishone) {
                YIO.io(url, {
                    ...
                    on: {
                        success: function (id, e) {
                            ...
                            yuisrc += e.responseText;
                            shareLibLogger.debug("Http GET request to " + url + " success!");
                        },
                        end: function (id, e) {
                            finishone();
                        },
                        failure: function (id, e) {
                            shareLibLogger.error("Http GET request to " + url + " failure,please check the url!");
                        }
                    }
                });
            }, function () {
                if (yuisrc.length > 0 && yuisrc !== "") {
                    ...
                    cb("Download yui runtime js error!" +
                        "Please check if your network has access to http://yui.yahooapis.com/combo?" +
                        opts.root + "/build/yui/yui-min.js ");
                }
            }
        );
    }
