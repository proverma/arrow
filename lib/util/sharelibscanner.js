...
var logger = log4js.getLogger("sharelibScanner");
...
function getArrowServerIPAddr() {
    ...
    try {
        ...
        logger.info("Arrow server status does not exist");
    }
...
function calcInstanceHashByFsPath(path) {
...
function isRegisteModuleDir(f) {
    var i = REGISTE_MODULES_DIR.length;
    while (i--) {
        if (REGISTE_MODULES_DIR[i] === f) {
            return true;
        }
    }
    return false;
...
exports.genSeedFile = function (scanFolder, callback) {
    ...
    var doscan = function (scanpath, finishone) {
        ...
        logger.info('Start scan share lib from : ' + fspath);

        try {
            if (!fs.statSync(fspath).isDirectory()) {
                logger.error('Please make sure the lib folder ' + fspath + ' exist!');
                showHelp();
                return finishone();
            }
        } catch (e) {
            logger.error('Please make sure the lib folder exist');
            showHelp();
            return finishone();
        }
    ...
    sync.forEachSeries(pathForScan, doscan, function () {
            logger.info("Total time :" + (new Date().getTime() - start) / 1000 + " s");
            //write modules info
...
function writeSeedFile(cb) {
    ...
    fs.writeFileSync(path.join(shareLibMetaPath, CLIENT_CONFIG_NAME), client_template.replace("%client%", config.client));
    fs.writeFileSync(path.join(shareLibMetaPath, SERVER_CONFIG_NAME), server_template.replace("%server%", config.server));
...
function generateMetaData(libname, libpath, done) {
    ...
    var libhash = calcInstanceHashByFsPath(libpath);
...
function captureYUIModuleDetails(filePath) {
    ...
    try {
        ...
        yui = null;
        logger.error('failed to parse javascript file ' + filePath + '\n' + e.message, 'error');
    }
    return yui;
}
