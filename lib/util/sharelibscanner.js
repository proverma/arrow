 ...
 * @return {String}
 */
scannerUtil.resolveYUIDenpendency = function resolveYUIDenpendency(yuideps, opts, callback) {
//    return fs.readFileSync(arrowConfig['yuiSandboxRuntime']);
    return (function () {
        var Y = require('yui').YUI(),
            ...
            loader = new Y.Loader({
                ...
                //And the root
                root: opts.root || "3.8.0/build/",
                //Require your deps
                require: yuideps
            });
        yuiout = loader.resolve(true);
        yuisrc += fs.readFileSync(arrowConfig['yuiSandboxRuntime']);
        if (yuiout && yuiout.js) {
            var YIO = require('yui/io-base'),
                tmpFile = path.join(shareLibMetaPath, "/yuiruntime_tmp.js");
            for (var i = 0; i < yuiout.js.length; i++) {
                YIO.IO.request(yuiout.js[i]).pipe(fs.createWriteStream(tmpFile));
                yuisrc += fs.readFileSync(tmpFile);
            }
        }
        return yuisrc;
    })();
