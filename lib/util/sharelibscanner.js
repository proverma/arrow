...
function captureMutipleYUIModuleDetails(filePath) {
    ...
    try {
        ...
        yreq = [];
        shareLibLogger.trace('Code in file: ' + filePath + ' is not written in standard YUI add method, we suggest to warp your code within YUI.add(\'...\') ' +
            'Error message: \n' + e.message);
    }
    return yreq;
}
...
function getShareLibSourcePath(libname, affnity) {
    ...
    function findSrcFromSeed(config, libname, affnity) {
        ...
        shareLibLogger.debug("-- Finding share lib path by libname: " + libname + " , affnity: " + affnity);
    ...
    if (foundLibs && foundLibs.length > 0)shareLibLogger.debug("-- Found share lib path: " + foundLibs);
    return foundLibs || [];
}
...
scannerUtil.getSrcDependencyByPath = function getSrcDependencyByPath(libpath, affnity) {
    ...
    if (shareLibsJs && shareLibsJs.length > 0) {
        ...
        for (i = 0; i < shareLibsJs.length; i++) {
            ...
            allDeps.yuiDepLibs = allDeps.yuiDepLibs.concat(yreq);
        }
    }
    return allDeps;
}
...
scannerUtil.resolveYUIDenpendency = function resolveYUIDenpendency(yuideps, opts, cb) {
    ...
    if (yuiversion !== Y.version) {
        shareLibLogger.info("Target yui version: " + yuiversion + " is not equal to local version: " + Y.version +
            " Will use iframe to dynamiclly inject yui modules");
        /*
         if here, the yui base: (yui-min,yui core modules )are ready,in case more yui runtime modules needed,we need
         Create a iframe to load dynamiclly modules,example is :
         (function () {
         // 1. all yui base goes here...
         // 2. var iframe = ...; // create an empty iframe element and insert it into the page
         // 3. create an instance bound to the iframe
         YUI({
         doc: iframe.contentWindow.document,  // this guarantees that any module required on demand will be injected in the iframe,
         and will be available to the sandbox automatically
         win: iframe.contentWindow
         }).use('test', 'simulate', 'whatever-other-too-you-need', function (Y) {
         // or just load the test script (without accessing any special module for features
         // and run the test using test runner
         });
         })();
         */
        var yui_min = 'http://yui.yahooapis.com/' + yuiversion + '/build/yui/yui-min.js';
        downloadjs(yui_min, function (error, src) {
            if (error) {
                shareLibLogger.error(error);
            } else {
                yuisrc += src.toString();
                yuisrc += "(function () { var iframe;" +
                    "iframe = document.body.appendChild(document.createElement(\'iframe\'));" +
                    "iframe.contentWindow.YUI=YUI;" +
                    " YUI.applyConfig({" +
                    "doc: iframe.contentWindow.document," + // this guarantees that any module required on demand will be injected in the iframe, and will be available to the sandbox automatically
                    "win: iframe.contentWindow" +
                    "});" +
                    "})();"
            }
            if (yuisrc.length > 0 && yuisrc !== "") {
                ...
                cb("Download yui seed js error!" +
                    "Please check if your network has access to " +
                    "http://yui.yahooapis.com/3.8.0/build/yui/yui-min.js ");
            }
        });

    } else {
        /*
         if we have same verison yui npm package then we can use yui loader to resolve modules info and then
         inject it into closure ,this could be faster then iframe loading modules from CDN.
         */
        loader = new Y.Loader({
        ...
        if (yuiout && yuiout.js) {
            ...
            sync.forEachSeries(yuiout.js, function (url, finishone) {
                    try {
                        ...
                        //if (/.*yui-base-min\.js/.test(url))url = url.replace("yui-base/yui-base-min.js", "yui/yui-min.js");
                        yuisrc += fs.readFileSync(path.normalize(url), 'utf8');
                        shareLibLogger.debug("Read file from " + url + " sucess!");
                        finishone();
                    } catch (e) {
                        url = CDN_BASE + url.replace(LOCAL_BASE, "");
                        shareLibLogger.debug("Read file " + url + " failed:\n" + e + ",\ntry send request to " + url);
