...
# --browser=chrome --driver=selenium
CMD=`$ARROWCI ./data/arrow_test/sharelib-test-unit-no-lib.js --browser=$BROWSERNAME --shareLibPath=./data/arrow_test/martini_lib/  --enableShareLibYUILoader=false --seleniumHost=$HUBHOST --capabilities=./data/arrow_test/cap.json | tail $TAILCOUNT | head $HEADCOUNT`
echo_and_save $CNT "$CMD"
if [ $? != 0 ]; then
{
    echo "ERROR!"
