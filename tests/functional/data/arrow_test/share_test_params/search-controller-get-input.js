...
SearchCustomController.prototype.execute = function(callback) {
    ...
    // check if testParams.shared of last controller can pass down
    self.logger.info("Searched: " + self.testParams.shared.suggestion);
    ...
    if(this.driver.webdriver){
        ...
        //Get searched query txt
        webdriver.findElement(webdriver.By.xpath('//*[@id="yschsp"]')).getAttribute("value").then(function (value) {
            self.testParams.shared.searched = value;
            callback();
        });
    } else {
